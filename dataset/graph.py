...
class PygGraph(object):
    ...
    def atom_dgl_multigraph(
        ...
        #ipdb.set_trace()
        sps_features = []
        for ii, s in enumerate(atoms.atomic_numbers):
            #feat = list(get_node_attributes(s, atom_features=atom_features))
            sps_features.append(s)
        sps_features = np.array(sps_features)
        node_features = torch.tensor(sps_features).type(
            torch.get_default_dtype()
        )
        edge_index = torch.cat((u.unsqueeze(0), v.unsqueeze(0)), dim=0).long()
        features = _get_attribute_lookup()
        node_features = torch.tensor(features[node_features.type(torch.IntTensor).squeeze()]).type(torch.FloatTensor)
